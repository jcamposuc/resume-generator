...
const ListItems = ({ items, handleRemove }) => (
    items?.map((item, index) => (
        <span key={`item-${index}`} className="inline-flex items-center px-2 py-1 text-sm font-medium text-gray-800 bg-gray-100 rounded me-2 dark:bg-gray-700 dark:text-gray-300">
            {item}
