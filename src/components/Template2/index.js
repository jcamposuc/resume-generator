...
const DivisorLine = ({ color = "bg-gray-300", height = "h-px" }) => (
    <hr className={`${height} mt-4 ${color} rounded-sm w-full`}></hr>
)

const SectionTitle = ({ title, hasLine = true }) => (
    <div className='flex mb-4'>
        <h3 className='mr-3 text-xl leading-3 uppercase'>{title}</h3>
        {hasLine && <hr className="h-0.5 bg-stone-500 rounded-sm w-full mt-4" />}
    </div>
)
...
const Header = ({ name, lastName, occupation, profilePicture, themeColor }) => (
    <header className={`pt-6 col-span-3 absolute w-full`}>
        <StyledDiv
            ...
            {...themeColor}>
            <div className="relative uppercase -top-3">
...
const ContactInformation = ({ phone, email, address, website, themeColor }) => (
    <section className='relative content-center p-3 pt-0'>
        <div className='mb-2'>
            <StyledSvg
                {...themeColor}
                className="inline-block w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div className='flex items-center'>
            <StyledSvg
                {...themeColor}
                className="w-6 h-6" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
